...
"""
    循环获取ip，用run_time来区分，跑完run_time同一置visit_Times的值，然后获取as信息，域名状态信息(visit_times标志as和state的获取)
    使用CNNICDNS获取域名NS记录，A记录和CNAME记录
...
def get_domains(limit_num = None):
    ...
    fetch_data = mongo_conn.mongo_read('domain_ip_cname',{'run_times':run_times},{'domain':True},limit_num)
...
def insert_data():
    ...
    while True:
        ...
        cur_array = 'domain_ip_cnames.' + str(run_times)

        try:
            # 更新visit_time的值
            # mongo_conn.mongo_inc(collection_name,{'domain':check_domain},{'visit_times':1})
            # mongo_conn.mongo_inc(collection_name,{'domain':check_domain},{'run_times':1})
            # 添加ip_cname的值
            # domain_ip_cnames = mongo_conn.mongo_update(collection_name,{'domain':check_domain},
            #                                             {cur_array:detect_res}
            #                                           )
            mongo_conn.mongo_any_update(collection_name,{'domain':check_domain},{'$set':{cur_array:detect_res},'$inc':{'run_times':1}})
